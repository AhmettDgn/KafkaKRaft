...
finish() {
  local code=$?
  if (( code != 0 )); then echo "Result: FAIL (exit $code)"; fi
}
trap finish EXIT
...
if "$test_restart"; then
  ...
  meta_before="$(kubectl exec -n "$namespace" "$pod" -c kafka -- cat /var/lib/kafka/data/meta.properties)"
  ...
  meta_after="$(kubectl exec -n "$namespace" "$pod" -c kafka -- cat /var/lib/kafka/data/meta.properties)"
